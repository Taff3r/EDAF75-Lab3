...
public class BookingPane extends BasicPane {
	...
	private void fillNameList() {
		...
		HashSet<String> names = db.getMovieNames();
		for(String s : names){
	...
	private void fillDateList() {
        ...
        /* --- insert own code here --- */
		String m_name = nameList.getSelectedValue();
		HashSet<String> dates = db.getPerformanceDates(m_name);
	...
	class DateSelectionListener implements ListSelectionListener {
		...
		public void valueChanged(ListSelectionEvent e) {
			...
			/* --- insert own code here --- */
			ShowStats stats = db.getPerformanceData(movieName, date);
			fields[0].setText(stats.m_name);
			fields[1].setText(stats.date);
			fields[2].setText(stats.t_name);
			fields[3].setText("" + stats.available_seats);
	...
	class ActionHandler implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if(db.bookTicket(movieName, date)) {
				displayMessage("Reservation successfull!");
				return;
			}
			displayMessage("No more seats");
		}
	}
}
